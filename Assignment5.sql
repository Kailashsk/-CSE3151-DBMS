...
No1.
a) select name, phone_no, cust_no from CUSTOMER natural join DEPOSITOR where DEPOSITOR.account_no = 'A0004';

   select name, phone_no, cust_no from CUSTOMER where cust_no in (select cust_no from DEPOSITOR where account_no = 'A0004');

b) select amount, branch_code from LOAN natural join CUSTOMER where CUSTOMER.name = 'YASH SARAF';

   select amount, branch_code from LOAN where cust_no in (select cust_no from CUSTOMER where name = 'YASH SARAF');

c) select CUSTOMER.name from CUSTOMER left join LOAN on CUSTOMER.cust_no = LOAN.cust_no where LOAN.cust_no is NULL;

  select name from CUSTOMER where cust_no not in (select cust_no from LOAN);

d) select account_no, balance from ACCOUNT natural join DEPOSITOR where DEPOSITOR.cust_no = 'C0010';

  select account_no, balance from ACCOUNT where account_no in (select account_no from DEPOSITOR where cust_no = 'C0010');

e) select distinct CUSTOMER.city from CUSTOMER self join CUSTOMER on CUSTOMER.name = 'ASLESHA TIWARI';

  select city from CUSTOMER where cust_no in (select cust_no from CUSTOMER where name = 'ASLESHA TIWARI');

f) select loan_no, branch_code,amount from loan natural join CUSTOMER where CUSTOMER.name = 'ANKITA SINGH';

  select loan_no, branch_code,amount from loan where cust_no in(select cust_no from customer where name = 'ANKITA SINGH');

g) select branch_name, branch_city from branch natural join customer where customer.name = 'ABHIJIT MISHRA';

select branch_name, branch_city from branch where branch_city in(select branch_city from customer where name = 'ABHIJIT MISHRA');

h) create table account_type (select account_no,type from account);

i) insert into account_type(select account_no,type from account where balance<50000);
